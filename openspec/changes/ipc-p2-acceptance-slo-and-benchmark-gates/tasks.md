...
- [ ] 1.1 审阅并确认 `模块级可验收标准` 的采样规模与统计口径
- [ ] 1.2 审阅并确认 Request-Response / Push / Zod 三类指标的独立验收路径
- [ ] 1.3 审阅并确认阈值违反时的门禁行为与输出报告字段
...
- [ ] 3.1 编写“RR 延迟阈值”失败测试并确认先失败
- [ ] 3.2 编写“Push 延迟阈值”失败测试并确认先失败
- [ ] 3.3 编写“阈值违规门禁阻断”失败测试并确认先失败

## 4. Green（最小实现通过）

- [ ] 4.1 实现 IPC acceptance 基准测试与统一统计器
- [ ] 4.2 实现阈值判定与报告输出
- [ ] 4.3 将门禁接入 CI，使新增 Red 用例转绿

## 5. Refactor（保持绿灯）

- [ ] 5.1 去重基准测试夹具与统计逻辑，保持测试全绿
- [ ] 5.2 保持阈值定义与输出格式稳定，避免报告解析破坏
