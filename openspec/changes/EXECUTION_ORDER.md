...
- 变更泳道：
  - Context Engine：`context-engine-p1 → (p2 || p3) → p4`（`context-engine-p0` 已归档）
...
1. Phase A（并行）
   - `context-engine-p1-token-budget-truncation`（依赖已归档 `context-engine-p0-layer-assembly-api`）
