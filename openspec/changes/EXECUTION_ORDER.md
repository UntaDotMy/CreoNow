...
2. `workbench-p5-04-command-palette` — 命令面板文件搜索 + 分类展示

### Phase C — 硬化收口（串行，依赖 Phase B 全部完成）

3. `workbench-p5-05-hardening-gate` — zod 校验、异常回退、去抖、NFR 验收、Storybook 补齐
...
archive/workbench-p5-00-contract-sync (Phase A baseline)
    ...
    └──→ workbench-p5-04-command-palette
               │
               ▼
...
- Change 03 与 Change 04 无互相依赖，可并行执行。
