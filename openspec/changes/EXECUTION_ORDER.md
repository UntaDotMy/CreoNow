...
- 变更泳道：
  ...
  - Version Control：`p2 → p3 → p4`（`p0`、`p1` 已归档）
...
3. `version-control-p2-diff-rollback`（依赖已归档 `version-control-p0` + 已归档 `version-control-p1` + 已归档 `editor-p2`）
...
7. `version-control-p4-hardening-boundary`（依赖已归档 `version-control-p0` + 已归档 `version-control-p1` + `version-control-p2` + `version-control-p3`）
...
- 跨泳道协同要求：
  - `version-control-p2` 进入 Green/Refactor 前需持续核对归档 `editor-p2` 的 DiffViewPanel / MultiVersionCompare 组件 API。
