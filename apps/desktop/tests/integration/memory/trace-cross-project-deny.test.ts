...
// Scenario Mapping: MS3-X-S2
{
  ...
  const traceService: MemoryTraceService = {
    getTrace: () => ({
      ...
      error: {
        ...
        message: "Cross-project trace access denied",
      },
    }),
    recordFeedback: () => ({
      ok: true,
  ...
  const response = (await traceGetHandler!(
    {},
    { projectId: "proj-2", generationId: "gen-1" },
  )) as IpcResponse<{ trace: unknown }>;

  // Assert
  assert.equal(response.ok, false);
  if (!response.ok) {
    assert.equal(response.error.code, "MEMORY_SCOPE_DENIED");
  }
}
